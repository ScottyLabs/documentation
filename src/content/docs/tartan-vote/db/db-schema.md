...
- [Sessions](db-schema/#sessions)
- [User Sessions](db-schema/#user-sessions)
...
- `data`: Other data related to the organization. See [organization.data](db-json/#organizationdata).
...
- `organization_id`: Primary Key. The id for an organization. References [Organizations](db-schema/#organizations).
- `user_id`: Primary Key. The id for the user in the organization. References [Users](db-schema/#users).
...
- `created_by_user_id`: The id of the user who created the session. Reference [Users](db-schema/#users).
...
- `user_id`: Foreign Key. The id for the user in the session. References [Users](db-schema/#users).
- `session_id`: Foreign Key. The id for the session. References [Sessions](db-schema/#sessions).
...
- `data`: The other data related to the event. See [event.data](db-schema/#eventdata)
- `created_by_user_id`. The id of the user who created the event. References [Users](db-schema/#users).
- `session_id`: The id for the session this event belongs to. References [Sessions](db-schema/#sessions).
...
- `event_id`: Foreign Key. The event this vote belongs to. References [Events](db-schema/#events).
- `user_session_id`: Foreign Key. The `user_session` row representing the participant/proxy vote instance. References [User Sessions](db-schema/#user-sessions).
- `cast_time`: The time at which this vote was cast (when the entry was created). Autogenerated.
- `data`: Other data pertaining to the vote. See [vote.data](db-json/#votedata)
