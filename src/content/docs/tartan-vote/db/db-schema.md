...
- `data`: Other data related to the organization. See [organization.data](/tartan-vote/db/db-json/#organizationdata).
...
- `organization_id`: Primary Key. The id for an organization. References [Organizations](#organizations).
- `user_id`: Primary Key. The id for the user in the organization. References [Users](#users).
...
- `created_by_user_id`: The id of the user who created the session. Reference [Users](#users).
...
- `user_id`: Foreign Key. The id for the user in the session. References [Users](#users).
- `session_id`: Foreign Key. The id for the session. References [Sessions](#sessions).
...
- `data`: The other data related to the event. See [event.data](#eventdata)
- `created_by_user_id`. The id of the user who created the event. References [Users](#users).
- `session_id`: The id for the session this event belongs to. References [Sessions](#sessions).
...
- `event_id`: Foreign Key. The event this vote belongs to. References [Events](#events).
- `user_session_id`: Foreign Key. The `user_session` row representing the participant/proxy vote instance. References [User Sessions](#user-sessions).
- `cast_time`: The time at which this vote was cast (when the entry was created). Autogenerated.
- `data`: Other data pertaining to the vote. See [vote.data](/tartan-vote/db/db-json/#votedata)
